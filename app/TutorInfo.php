...

{
    ...
    public function getPostList($postcode) {
    ...
    public function getPostListfromArray($postcodelist) {

        $postlistarray   = [];

        for($i=0; $i < count($postcodelist); $i++){
            $postcodes = new Postcode();
            $singlepost = $postcodes->checkAndUpdatePostcode($postcodelist[$i]);
    ...
    public function searchFilter($data){
        ...
        if($data['postcode'] != null) {
            $postcodes = $this->getPostList($data['postcode']);
        }
    	...
    	if($data['price'] != null && $data['tutortype'] != null && $data['price'] != $data['tutortype']){
	    	...
	    	$query = DB::table('tutorinfo')->select('tutorinfo.*','tutorexperience.*','coursesnames.coursename','tutortotime.*', 'tutoravgrating.avgrating','users.useractive')
	                  ...
	                  if($data['sortitem'] != null){
		                  ...
		                  $query->orderBy('tutoravgrating.avgrating','DESC');

		                }elseif($data['sortitem'] == 'experience'){

                          $query->orderBy('tutorinfo.experience', 'DESC');
                      ...
                      if($data['experience'] != null && $data['experience'] == 3){

                        $query->where(['experience' => $data['experience']]);

                      } elseif($data['experience'] != null && $data['experience'] == 2){
                        $query->whereIn('experience',[1,2]);
                      }
                      ...
                      if($data['subjects'] != null || $data['classes'] != null){
	                   ...
	                   $query->where(['users.useractive' => 1]);
                       if($postcodes != null) {
                        $query->whereIn('tutorinfo.postalcode',$postcodes);
                       }    
                       
	                   $tutorinfo = $query->paginate(50);  
    ...
    public function getUserFromCourseClasses($data){
    	...
    	if($data['subjects'] != null && $data['classes'] != null){
            $query = DB::table('coursesnames');
    		for($i = 0; $i < count($data['subjects']); $i++){

    			$query = $query->where('coursename', 'like', '%'.$data['subjects'][$i] . '%');
    		}
            $tutorcourse = $query->where('classes', 'like', '%'. $data['classes'].'%')->get();
            if($tutorcourse){
